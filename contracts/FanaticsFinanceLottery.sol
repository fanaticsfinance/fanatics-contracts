...
import "./interfaces/IFanaticsFinanceLottery.sol";

/** @title FanaticsFinance Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract FanaticsFinanceLottery is ReentrancyGuard, IFanaticsFinanceLottery, Ownable {
    ...
    uint256 public maxPriceTicketInGoal = 50 ether;
    uint256 public minPriceTicketInGoal = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInGoal;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] goalPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInGoal;
        uint32 finalNumber;
    }
     ...
     * @param _goalTokenAddress: address of the GOAL token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _goalTokenAddress, address _randomGeneratorAddress) {
        ...
        nonReentrant
    {
        ...
        // Calculate number of GOAL to this contract
        uint256 amountGoalToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInGoal,
            _ticketNumbers.length
        );

        // Transfer goal tokens to this contract
        goalToken.safeTransferFrom(address(msg.sender), address(this), amountGoalToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInGoal += amountGoalToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInGoalToTransfer
        uint256 rewardInGoalToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInGoalToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        goalToken.safeTransfer(msg.sender, rewardInGoalToTransfer);

        emit TicketsClaim(msg.sender, rewardInGoalToTransfer, _lotteryId, _ticketIds.length);
    }
    ...
    /**
     * @notice Draw the final number, calculate reward in GOAL per group, and make lottery claimable
     * @param _lotteryId: lottery id
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInGoal) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in GOAL for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].goalPerBracket[j] =
                ...
                // A. No GOAL to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].goalPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInGoal - amountToShareToWinners);

        // Transfer GOAL to treasury address
        goalToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in GOAL token
     * @dev Callable by owner or injector address
     */
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not open");

        goalToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInGoal += _amount;
     ...
     * @param _priceTicketInGoal: price of a ticket in GOAL
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInGoal >= minPriceTicketInGoal) && (_priceTicketInGoal <= maxPriceTicketInGoal),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInGoal: _priceTicketInGoal,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            goalPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInGoal: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(goalToken), "Cannot be GOAL token");
    ...
    /**
     * @notice Set GOAL price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInGoal: minimum price of a ticket in GOAL
     * @param _maxPriceTicketInGoal: maximum price of a ticket in GOAL
     */
    function setMinAndMaxTicketPriceInGoal(uint256 _minPriceTicketInGoal, uint256 _maxPriceTicketInGoal)
        external
        onlyOwner
    {
        require(_minPriceTicketInGoal <= _maxPriceTicketInGoal, "minPrice must be < maxPrice");

        minPriceTicketInGoal = _minPriceTicketInGoal;
        maxPriceTicketInGoal = _maxPriceTicketInGoal;
    }
     ...
     * @param _priceTicket price of a ticket (in GOAL)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].goalPerBracket[_bracket];
        } else {
            return 0;
        }
    }
